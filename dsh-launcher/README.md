...
2. **更新检查**（仅冷启动且当前在 `dev` 分支；同终端内重启不检查）— 先**快速探测**仓库所在主机（3 秒限时，走 DNS+TCP+TLS，与 git 相同路径；不用 ping 是因为 ICMP 与 443 命运常不一致且无法探测代理）：直连不可达 → 自动经代理 `127.0.0.1:7897` 探测（3 秒）；两者都不可达则**跳过更新**（最坏仅约 6 秒），用现有代码继续启动，绝不阻塞。探测通过后才正式 `git fetch`（直连 8 秒/代理 10 秒限时兜底）。有更新时对 `origin/master` 执行 `git merge --ff-only` 快进合并——工作区有未提交修改、本地有独立提交/已分叉、或当前不在 `dev` 分支时自动跳过，绝不强制覆盖
3. **条件构建** — 仅当刚更新了代码、构建记录与当前提交不一致、或 `apps/web/dist` 缺失时，才执行 `pnpm install --frozen-lockfile && pnpm run build`（自动构建不改写 lockfile）；构建失败会自动回退到更新前的提交，用旧版本启动
...
- 更新一律使用 `git merge --ff-only`（仅在当前处于 `dev` 分支时快进合并
  `origin/master`），工作区有未提交修改、本地有独立提交/已分叉、或不在
  `dev` 分支时自动跳过更新，**绝不**强制覆盖或推送。唯一例外是「更新后
  构建失败」的回退会执行一次 `reset --hard` 回到更新前提交——此时合并前
  已验证工作区干净，回退不会丢失任何本地修改
- 自动构建使用 `pnpm install --frozen-lockfile`，构建过程不改写 lockfile；
  lockfile 与 package.json 不一致时构建直接失败并触发回退
