...
import { AUTO_WIDEN, PANEL_DEFAULT, PANEL_MIN, WIDTH_KEY, clampPanelWidth, cubicBezierEase, messageOf, panelActionFor, readStored, writeStored } from './helpers.js'
...
export function WorkbenchPanel(props) {
  ...
  const openFeature = useCallback((featureId, opts) => {
    ...
    // 单实例标签常驻不可关闭；closable 元数据为 false 时同样禁止关闭。
  ...
  const refresh = () => {
    ...
    setRefreshing(true)
    Promise.resolve(path !== undefined ? loadGitState(path, undefined, showIgnored) : undefined)
