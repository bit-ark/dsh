/**
 * dsh-archive — 宿主半（Host）。
 *
 * 在 DSH web 服务上注册两个 JSON 端点（路径前缀 /dsh-archive）：
 *
 *  - POST /dsh-archive/restore  { sessionId }
...
export function apply(ctx: any): void {
  const handler = async (req: IncomingMessage, res: ServerResponse): Promise<void> => {
    ...
    const route = pathname === '/dsh-archive/restore'
      ? 'restore'
      : pathname === '/dsh-archive/delete'
        ? 'delete'
        : pathname === '/dsh-archive/delete-all'
          ? 'delete-all'
          : undefined
    try {
