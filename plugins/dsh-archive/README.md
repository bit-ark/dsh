...
- **面板**：点击按钮在 footer 上方打开浮动面板，支持 Escape / 点击外部关闭。
  ...
  结果提示区显示最近一次删除/恢复的结果或错误明细（批量删除总是给出完整
  汇总）；提示随面板生命周期走——关闭面板（✕ / Escape / 外点）即清除，
  重开面板是干净的。删除动作保留点击时的 `window.confirm` 二次确认。
...
- **一键删除**：面板底部 footer 区域的按钮，确认后把当前项目全部已归档会话一次性
  ...
  被跳过的会话 id 并解释「重启 dsh web 后可删除」；提示随面板关闭清除，重开干净。
