...
为 DeepSeek Harness Web GUI 的设置面板增加「归档」页（`settings.section` 官方插槽的
一个 list 条目 = 设置左侧导航菜单项 + 整页内容区）。

归档是项目级管理（workspace 浏览器右键「归档会话」→ 会话消失；恢复 → 会话回到
列表），放在设置面板的独立页面而不是会话区的 tab（会话 tab 行是 session 作用域
的视图，作用域不匹配，且没有打开会话时不可达）；侧边栏 footer 因此腾给了
「DeepSeek 账户」小部件（dsh-balance）。

- **入口**：设置面板左侧导航的「归档」菜单项（order 30）。页面打开时渲染当前
  项目全部已归档会话；切换设置 section 或关闭设置面板后页面卸载，再进来是干净的
  新页面（结果提示随之清除）。
...
- **下载**：每行「下载」按钮对接 harness 核心端点 `/api/session.export`，把该会话
  日志作为 ZIP 通过浏览器下载管理器导出；下载状态存本插件私有 store，行空闲渲染
  不丢状态（下载中 → 成功 / 失败重试）。
- **一键删除**：页面底部按钮，确认后把当前项目全部已归档会话一次性批量硬删除——
  复用单删的同一套步骤（逐会话），运行中的会话跳过、单个硬错误不中断整批。
  **总是显示完整结果提示**（已删除 / 跳过运行中 / 失败 + 明细），跳过时列出被跳过
  的会话 id 并解释「重启 dsh web 后可删除」；提示随页面卸载清除，重进干净。
  删除动作保留点击时的 `window.confirm` 二次确认。

## 结构

```
src/index.ts              宿主半：POST /dsh-archive/restore | /delete | /delete-all
src/client/index.tsx      客户端半：settings.section slot 注册（id: dsh-archive, order: 30）
...
- 已归档但所属 workspace 已删除的"孤儿"会话（目录不存在）不属于任何项目，
  任何项目的归档页都不会显示它们；重新注册原目录为 workspace 后即可管理。
