 ...
 * toast 直达详情弹窗——这是 dsh-notify 的业务闭环示例。notifier 缺席时
 * 本插件功能不受影响。
 */
...
`;
		...
		if (typeof document !== "undefined" && document.getElementById("dsh-update-style") === null) {
			const styleEl = document.createElement("style");
			styleEl.id = "dsh-update-style";
			styleEl.setAttribute("data-plugin", "dsh-update");
					...
					.then((data) => {
						...
						if (data.state === "update" && typeof props.getNotifier === "function") {
							...
							if (notifier !== undefined && typeof notifier.notify === "function") {
								notifier.notify({
									id: `dsh-update-${data.remoteSha || "unknown"}`,
									title: "发现新版本",
		...
		exports.apply = function apply(ctx) {
			...
			ctx.effect(
				...
				"dsh-update: settings row registration",
			);
		};
