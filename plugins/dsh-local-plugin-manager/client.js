/**
 * dsh-local-plugin-manager — client half (browser plane).
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── 样式：一张带标记的样式表（与 dsh-updater 同法），全部用主题 token ──
		const CSS = `
...
`;
		if (typeof document !== "undefined" && document.getElementById("lpm-style") === null) {
			...
			styleEl.setAttribute("data-plugin", "dsh-local-plugin-manager");
		...
		exports.name = "dsh-local-plugin-manager";
		...
		exports.apply = function apply(ctx) {
			...
			ctx.effect(
				...
				"dsh-local-plugin-manager: tab registration",
			);
		};
