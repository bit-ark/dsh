/**
 * dsh-notify — 客户端半（Browser）：通用应用内通知。
 *
 * 本插件是「服务 + UI」库：
 ...
 *
 * 业务闭环示例：dsh-update 在检测到新版本时调用 notifier.notify(...) 弹 toast，
...
`;
		...
		if (typeof document !== "undefined" && document.getElementById("dsh-notify-style") === null) {
			const styleEl = document.createElement("style");
			styleEl.id = "dsh-notify-style";
		...
		function showToastOnce(entry) {
			const KEY = "dsh-notify-toast-seen";
			let seen = {};
		...
		exports.apply = function apply(ctx) {
			...
			}, "dsh-notify: toast host + bell registration");
		};
