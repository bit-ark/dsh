 ...
 *  - 恢复：POST /dsh-archive-tab/restore——宿主把会话移出归档集合；随后广播
...
export function apply(ctx: any): void {
  ...
  // 与 dsh-updater 同样的约定（id + data-plugin）；守卫防止重复挂载重复插标签。
  const STYLE_ID = 'dsh-archive-tab-style'
  if (typeof document !== 'undefined' && document.getElementById(STYLE_ID) === null) {
    const styleEl = document.createElement('style')
    styleEl.id = STYLE_ID
    styleEl.setAttribute('data-plugin', 'dsh-archive-tab')
    styleEl.textContent = `
      .dsh-archive-badge {
