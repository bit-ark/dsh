...
cordis.patch.yml  自带组合层：插入 dsh-updater 行（config: remote/branch）
```
...
- 远端版本尽力而为：raw.githubusercontent.com 不可达时 remoteVersion 为 null，
  弹窗仍给出远端 sha 与手动指引。
- 对 dsh-notifier 的消费是可选、特性探测的：notifier 缺席时仅无 toast，
  其余功能不变。
