...
- **消费部分颜色标识当前时段**：低谷时段 = 成功绿
  ...
  小时 ∈ {9..11, 14..17}，其余为低谷；**2026-08-23 起周末（周六、周日）全天不再
  区分峰谷、统一按低谷价**；峰谷按北京时间计时、全球统一适用；每分钟 tick 一次，
  峰谷边界到点自动变色。官方 usage 接口只按「缓存命中/未命中/输出」
...
- `GET /dsh-balance/usage?days=N&refresh=1`
  ...
  高峰时段的样本按 ×2 计；2026-08-23 起周末全天按低谷价，priced = 今日是否有
  可计价样本），供其他消费方使用；
...
**更省事的方式**：无需改配置文件——token 未配置或过期时，侧边栏小部件会
自动变成输入框，直接粘贴 userToken 点「保存」即可（保存到 credentials 文件，
立即生效，无需重启；若 token 无效会提示「Token 无效或已过期」并保留输入框）。

注意：userToken 是账号会话凭证，会过期；过期后小部件自动弹出输入框，重新
登录复制粘贴即可（凭证失效错误不缓存，无需等 5 分钟缓存）。若当前 token 由
环境变量提供（只读层），保存会提示改为在环境变量中更新。
